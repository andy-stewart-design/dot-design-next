...
Not all SVGs are rendered equal.

[SVG](https://www.w3.org/TR/SVG2/), like most of the features and technologies we use in our work, is a specification—a standardized set of principles, which were written out and agreed upon, and then then interpretted and implemented by a variety of parties over time. This process is ultimately a good faith agreement, meaning that any individual renderign engine may or may not be complete and standard-compliant.

In a time with such a large emphasis on parity, it’s a truth that can be easy to forget. That is, until it isn’t.
