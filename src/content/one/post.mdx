...
This is where we talk about finding a specific point along the circumfrence of a circle.
...
One note about the above code: you may have noticed that our mask is wrapped in a `defs` element. This element simply allows us to store anything that we need to reference in our SVG, but that we do not want to render directly.

Check out the playground below to explore the various dimensions of masking in SVGs.
