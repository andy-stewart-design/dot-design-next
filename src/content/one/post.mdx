...
At its most basic, a sine function calculates the distance from the center line of any point along a sine wave. Because sine functions operate on a unit circle, the distance calculated will always be between -1 and 1. To find this distance, the sine function needs to know the angle, in radians, of the point we want to calculate, which we can find by multiplying the circumference of the unit circle by some percentage. In practice, you might express that like this:
